...
function FlowchartCanvas() {
  ...
  const hitTestElement = useCallback((worldX, worldY) => {
    ...
    for (let i = visibleElements.length - 1; i >= 0; i--) {
      ...
      // Determine size based on type and content to match rendering
      let size = BASE_SIZE;
      if (isDepartment) {
        // Check if has children - consistent with drawElement
  ...
  const findDropTarget = useCallback((worldX, worldY, excludeId) => {
    ...
    for (const element of visibleElements) {
      ...
      // Determine size based on content to match rendering
      let size = BASE_SIZE;
      const hasChildren = elements.some(e => e.parentId === element.id);
      if (hasChildren) {
        size = BASE_SIZE * 1.4;
  ...
  // Handle wheel for zoom - manual attachment to avoid passive event listener issues
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;

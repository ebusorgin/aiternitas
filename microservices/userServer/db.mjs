...
const { Pool } = pg;
console.log("ssssssssssssssssssssssssssss",process.env.DB_HOST)
const resolvedDbUser =
  process.env.DB_USER || process.env.PGUSER || process.env.USER;

const dbConfig = {
  host: process.env.DB_HOST, // Дефолт для Open Server
...
export async function initDatabase() {
  try {
    ...
    const adminDbConfig = {
      host: process.env.DB_HOST || '127.0.0.1', // Дефолт для Open Server
      port: parseInt(process.env.DB_PORT || '5432', 10),
      database: 'postgres',
    };
    ...
    // Таблица system_plugins для хранения сгенерированных скриптов (Эволюция)
    await pool.query(`
      CREATE TABLE IF NOT EXISTS system_plugins (
        ...
        docker_image VARCHAR(100) DEFAULT 'python:3.10-alpine',
    ...
    `);
    console.log('✅ Таблица system_plugins создана/проверена');

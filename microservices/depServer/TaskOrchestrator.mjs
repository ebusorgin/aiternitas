...
import { callLocalLLM } from './utils/aiClient.mjs';
import { publishTask } from './utils/amqpClient.mjs';
...
Верни СТРОГО JSON: { "action": "use_existing", "department_id": "ID" } ИЛИ { "action": "create_new", "department_name": "Название нового отдела" }`;
      ...
      let deptResult;
      try { deptResult = typeof deptLLM === 'string' ? JSON.parse(deptLLM) : deptLLM; } catch(e) { deptResult = { action: 'create_new', department_name: 'Отдел разработки' }; }
...
Верни СТРОГО JSON: { "action": "use_existing", "worker_id": "ID" } ИЛИ { "action": "create_new", "worker_name": "Имя (например Python-программист)", "ai_prompt": "Краткая инструкция, например: Ты пишешь python скрипты" }`;
      ...
      let workerResult;
      try { workerResult = typeof workerLLM === 'string' ? JSON.parse(workerLLM) : workerLLM; } catch(e) { workerResult = { action: 'create_new', worker_name: 'AI-Разработчик', ai_prompt: 'Ты опытный программист.' }; }
      ...
      // 3. ПЕРЕДАЧА ЗАДАЧИ В ОЧЕРЕДЬ (RABBITMQ)
      await pool.query(
        "UPDATE tasks SET assigned_to_worker_id = $1 WHERE id = $2",
        [targetWorkerId, task.id]
      );

      console.log(`[TaskOrchestrator] Публикация задачи #${task.id} в RabbitMQ (tasks_queue) для работника ${targetWorkerId}`);
      await publishTask('tasks_queue', {
        taskId: task.id,
        workerId: targetWorkerId,
        workerPrompt: targetWorkerPrompt,
        deptId: targetDeptId,
